    ...
    if (empty($username) || empty($email) || empty($phone) || empty($role)) {
        ...
        $sql = "
            ...
            SET 
                ...
                S_PHONENUM = :phone,
                S_ROLE = :role" .
                (!empty($password) ? ", S_PASS = :password" : "") . "
        ...
        if ($result) {
            ...
            $e = oci_error($stmt);
            $error = "Error updating staff: " . $e['message'];
        }
    ...
    <div class="p-8">
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-pink-700 text-3xl font-bold">Edit Staff Profile</h2>
                <a href="a.view_staff.php?id=<?php echo htmlspecialchars($staffId); ?>" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Back to View</a>
            </div>
                ...
                <form method="POST" class="space-y-6">
                    <div>
                        ...
                        <input type="text" id="username" name="username" 
                            value="<?php echo htmlspecialchars($staff['S_USERNAME']); ?>"
                    ...
                    <div>
                        ...
                        <select id="role" name="role" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-pink-500 focus:border-pink-500"
                            required>
                            <option value="Admin" <?php echo $staff['S_ROLE'] === 'Admin' ? 'selected' : ''; ?>>Admin</option>
                            <option value="Staff" <?php echo $staff['S_ROLE'] === 'Staff' ? 'selected' : ''; ?>>Staff</option>
                        </select>
                    </div>

                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700 mb-1">New Password (leave empty to keep current)</label>
